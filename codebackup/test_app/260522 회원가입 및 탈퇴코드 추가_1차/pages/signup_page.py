...
class SignUpPage(BasePage):
    ...
    def click_agree_checkboxes(self):
        if self.platform == "aos":
            ...
            assert self.is_displayed(AOS_SignUpLocators.AGREE_CHECK), "❌ 선택 약관 동의 체크 실패"
            self.click(AOS_SignUpLocators.AGREE_SECOND)
            assert not self.is_displayed(AOS_SignUpLocators.AGREE_CHECK, timeout=0.5), "❌ 개인정보 수집동의 체크 실패"
        else:
            self.click(IOS_SignUpLocators.AGREE_FIRST)
            assert self.is_displayed(IOS_SignUpLocators.AGREE_CHECK), "❌ 선택 약관 동의 체크 실패"
            self.click(IOS_SignUpLocators.AGREE_SECOND)
            assert not self.is_displayed(IOS_SignUpLocators.AGREE_CHECK, timeout=0.5), "❌ 개인정보 수집동의 체크 실패"
    ...
    def open_url_in_browser_pc(self, url: str):
        webbrowser.open(url)
        print(f"[open_url_pc] PC 크롬에서 URL 열기: {url}")
    ...
    def is_signup_complete_displayed(self) -> bool:
        ...
        self.wait_for_native()
        if self.platform == "aos":
            return self.is_displayed(AOS_GenrehomeLocators.WEBTOON_TAB)
        else:
            return self.is_displayed(IOS_GenrehomeLocators.WEBTOON_NEW_QUICK)
...
class WithdrawPage(BasePage):
    ...
    def click_ios_withdraw_account(self):
