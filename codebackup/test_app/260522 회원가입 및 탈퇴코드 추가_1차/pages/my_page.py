...
class MyInfoPage(BasePage):
    ...
    def is_recheck_pw_title_displayed(self) -> bool:
        ...
        if not result:
            print("[SKIP] 비밀번호 재확인 타이틀 미노출")
        return result

    def input_recheck_pw(self, password: str):
        if self.platform == "aos":
            if self.is_displayed(AOS_MyInfoLocators.RECHECK_PW_INPUT, timeout=3):
                self.find_element(AOS_MyInfoLocators.RECHECK_PW_INPUT).send_keys(password)
            else:
                print("[SKIP] 비밀번호 재확인 입력 필드 미노출")
        else:
            if self.is_displayed(IOS_MyInfoLocators.RECHECK_PW_INPUT, timeout=3):
                self.find_element(IOS_MyInfoLocators.RECHECK_PW_INPUT).send_keys(password)
            else:
                print("[SKIP] 비밀번호 재확인 입력 필드 미노출")
    ...
    def get_current_user_id(self) -> str:
        ...
        print(f"[current_user_id] 현재 계정 ID: {user_id}")
        return user_id
