...
class TestContentsHome_AllAges:
    ...
    def handle_episode_entry_flow(self, episode_desc: str, has_ownership: bool = False):
        ...
        self.viewer.dismiss_another_device_alert_if_present()
    ...
    def test_select_episode_allages(self):
        if TestContentsHome_AllAges.viewer_auto_entered:
            if self.viewer.is_all_viewer_displayed():
                logging.info("[SKIP] 뷰어 자동진입 확인 - 스킵")
                return
            logging.info("[INFO] viewer_auto_entered=True이나 뷰어 미감지 - 재처리")
        if self.viewer.is_all_viewer_displayed():
...
class TestCart:
    ...
    def test_App_Checklist_095_카트진입(self):
        ...
        time.sleep(5)
        logging.info(f"[카트진입] current_context: {self.driver.current_context}")
        logging.info(f"[카트진입] all_contexts: {self.driver.contexts}")
        assert self.page.is_owntab_displayed(timeout=NETWORK_TIMEOUT), "❌ 카트 화면 진입 실패"
...
class TestMyridi:
    ...
    def test_entered_chargecash_popup(self):
        ...
        time.sleep(3)
        assert self.page.is_chargepopup_entered(timeout=NETWORK_TIMEOUT), "❌ 리디캐시 충전화면 진입 실패"
