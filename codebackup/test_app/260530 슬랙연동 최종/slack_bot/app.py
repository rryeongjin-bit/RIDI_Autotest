...
# 테스트 모듈 목록 (추가 시 여기에만 추가)
TEST_MODULES = [
    {"value": "tests/test_basic.py", "label": "test_basic.py"},
    #{"value": "tests/test_p1.py",    "label": "test_p1.py"},
]
...
def slash_stop():
    ...
    trigger_id = request.form.get("trigger_id")
    _open_stop_modal(trigger_id)  # 항상 모달 오픈
    return "", 200
    ...
    if runner.stop_test(run_id):
        ...
        if run_info:
            _update_main_message(
                ...
                ts=run_info["ts"],
                env_label=run_info["environment"],  # ← environment → env_label
                status="🛑 테스트 강제 종료",
                run_id=run_id,
            )
            _post_thread(
...
def _execute_tests(run_id, platform, modules, env_selected, env_label, parallel, channel, ts, thread_ts):
    ...
    def on_log(log_chunk: str):
        ...
        # 진행률 라인 (PASSED/FAILED/SKIPPED)인지 확인
