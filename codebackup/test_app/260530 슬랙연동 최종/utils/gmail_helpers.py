...
def _retry_with_force_refresh(after_timestamp=None, sender_email=None, sign_up_email=None):
    logging.info("🔄 토큰 강제 재갱신 시도...")
    clear_token()
    if force_refresh_token():
        logging.info("✅ 토큰 강제 재갱신 성공. Gmail API 재시도합니다.")
        try:
...
def get_latest_verification_url(after_timestamp=None, sender_email=None, sign_up_email=None):
    try:
        ...
        logging.warning(f"⚠️ 토큰 갱신 실패: {e}")
        return _retry_with_force_refresh(after_timestamp, sender_email, sign_up_email)
    except HttpError as e:
        if e.resp.status in (401, 403):
            logging.warning(f"⚠️ Gmail API 인증 오류 (HTTP {e.resp.status}): {e}")
            return _retry_with_force_refresh(after_timestamp, sender_email, sign_up_email)
        logging.warning(f"⚠️ Gmail API HTTP 오류: {e}")
        return None
    except Exception as e:
        logging.error(f"⚠️ Gmail API 예외 발생: {e}")
        return None