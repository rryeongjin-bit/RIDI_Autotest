...
class TestContentsHome_AllAges:
    ...
    def test_App_Checklist_177_다운로드아이콘(self):
        ...
        if has_ownership:
            logging.info("오너십 보유 - 다운로드 완료 대기")
            ...
            TestContentsHome_AllAges.viewer_auto_entered = (
                not TestContentsHome_AllAges.has_pay_popup
            )

        logging.info(f"has_webview 결과: {TestContentsHome_AllAges.has_pay_popup}")
        logging.info(f"contexts: {self.driver.contexts}")
    ...
    def test_App_Checklist_423_회차구매(self):
        if TestContentsHome_AllAges.viewer_auto_entered:
            logging.info("[SKIP] 다운로드 후 뷰어 자동 진입 상태 - 회차구매 스킵")
            return

        if not TestContentsHome_AllAges.has_pay_popup:
            logging.info("오너십 보유 및 다운로드 완료 - 회차 선택")
    ...
    def test_App_Checklist_423_뷰어진입(self):
    ...
    def test_App_Checklist_388_다음화결제(self):
        ...
        logging.info(f"첫 번째 타이틀: {first_title}")
        ...
        logging.info(f"두 번째 타이틀: {second_title}")
...
class TestContentsHome_Adult:
    ...
    def test_App_Checklist_177_다운로드아이콘(self):
        ...
        if has_ownership:
            ...
            TestContentsHome_Adult.viewer_auto_entered = (
                not TestContentsHome_Adult.has_pay_popup
            )

        logging.info(f"has_webview 결과: {TestContentsHome_Adult.has_pay_popup}")
        logging.info(f"contexts: {self.driver.contexts}")
        logging.info(f"뷰어 자동 진입 여부: {TestContentsHome_Adult.viewer_auto_entered}")

    def test_App_Checklist_181_회차구매(self):
        if TestContentsHome_Adult.viewer_auto_entered:
            logging.info("[SKIP] 다운로드 후 뷰어 자동 진입 상태 - 회차구매 스킵")
            return

        if not TestContentsHome_Adult.has_pay_popup:
            logging.info("오너십 보유 및 다운로드 완료 - 회차 선택")
    ...
    def test_App_Checklist_181_뷰어진입(self):
