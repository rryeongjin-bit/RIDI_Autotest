import os
import re       # [추가] summary 내 실패 라인 파싱용
import json
import hmac
import hashlib
import time
...
def _execute_tests(run_id, platform, modules, env_selected, env_label, parallel, channel):
    ...
    def on_finish(platform_key: str, returncode: int, summary: str, all_failures: list = None):
        ...
        _update_main_message(
            ...
            run_id=sub_run_id,
            platform_label=platform_key.upper(),
            # [변경] fs_lines 추가 전달 → short test summary info 대신 fs_lines 기반 실패 내역 출력
...
# [변경] summary 내 실패 내역 포맷 정제 함수
# - 기존: short test summary info 섹션에서 FAILED/ERROR 경로만 파싱하여 출력
# - 변경: short test summary info 섹션 제거 후 fs_lines(실시간 수집 데이터)로 대체
#   → fs_lines에는 FAILED / SKIPPED 모두 포함되어 있어 스킵 케이스도 출력 가능
#   → 항목 끝의 진행률 "[XX%]" 부분은 제거하여 깔끔하게 출력
# - 공통: = FAILURES = / = ERRORS = 섹션(스택트레이스 전체)은 그대로 제거
def _format_summary_failures(summary: str, fs_lines: list = None) -> str:
    ...
    for line in lines:
        # = FAILURES = 또는 = ERRORS = 섹션 시작 → 스킵 모드 진입
        if re.search(r'=+\s*(FAILURES|ERRORS)\s*=+', line):
            in_failures = True
            continue

        # short test summary info 섹션 시작 → 스킵 모드 진입 (fs_lines로 대체)
        if 'short test summary info' in line:
            in_failures = False
            in_short_summary = True
            continue

        # FAILURES/ERRORS 섹션 내 모든 라인 스킵 (스택트레이스)
        if in_failures:
            continue

        if in_short_summary:
            if re.match(r'(?:FAILED|ERROR)\s+', line):
                # short test summary info의 FAILED/ERROR 라인 스킵 (fs_lines로 대체)
                continue
            else:
                # short test summary 섹션 종료 → fs_lines 기반 실패 내역 삽입
                in_short_summary = False
    ...
    # short test summary 섹션이 파일 끝까지 이어진 경우 처리
    if fs_lines and not inserted:
        clean = [re.sub(r'\s*\[.*?\]\s*$', '', l).strip() for l in fs_lines]
        result.append('실패 내역 :')
        result.extend(clean)
