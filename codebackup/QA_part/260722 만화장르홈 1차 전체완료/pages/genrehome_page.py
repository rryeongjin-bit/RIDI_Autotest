...
class WebtoonGenrePage(BasePage):
    """웹툰 장르홈 - ComicGenrePage와 동일 구조(요소값만 다름)이나, 프로젝트 컨벤션에 따라
    독립된 로케이터 클래스(AOS_WEBTOON_GENRE/IOS_WEBTOON_GENRE)를 참조하는 별도 페이지
    오브젝트로 분리한다. 우선 진입 확인에 필요한 최소 구성만 두고, 이후 실기기 확인 후
    로케이터/메서드를 보완한다."""
    SUBTAB_LOCATOR = {
        "추천": "SUBTAB_RECOMMEND",
    }
