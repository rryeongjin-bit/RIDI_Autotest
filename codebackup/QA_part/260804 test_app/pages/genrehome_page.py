...
class ComicGenrePage(BasePage):
    ...
    def verify_quickmenu_destination_title(self, menu_name: str, timeout: int = 6, interval: float = 1.0) -> bool:
        ...
        대기를 5초 → 8초 → 10초로 늘렸다(2026-08-03 사용자 지시). 도서 장르홈 "리디온리"처럼
        목적지가 프로모션 지면인 경우 타이틀 렌더링이 늦어, 같은 값이 한 실행에서는 정상 확인되고
        (17:01 실제:'[5% 포인트백] 당신에게 가장 먼저🎁 RIDI ONLY 도서' ✅) 다른 실행에서는
        빈 문자열로 읽혀 실패했다(17:10 실제:'' ❌). 값이 틀린 게 아니라 아직 안 그려진 것이라
        대기만 늘리면 해소된다. 8초로도 타임아웃이 재발해 10초로 올렸다."""
        time.sleep(10)
    ...
    def _vertical_swipe_up(self):
        ...
        iOS는 원시 스와이프를 쓰지만 시작 y를 0.90에서 아래 SECTION_SEARCH_STEP_START_RATIO
        (0.78)로 낮춘다. 0.90(390x844 기준 y 759)은 하단 네비게이션바 영역이라 제스처가
        시스템에 먹혀 스크롤이 불안정하다 - 같은 화면에서 3회 시도 중 1회만 이동하는 것이
        실기기로 확인됐다(2026-08-03 취향저격 AI추천 섹션. 0.78→0.32로 바꾸자 4회만에 푸터
        도달). 이 함정은 바로 아래 SECTION_SEARCH_STEP_* 주석에 이미 기록돼 있었는데
        (_section_search_scroll_up은 그때 고쳐졌다) 이 함수에는 반영되지 않아 남아 있었다.
        AOS는 UiScrollable 분기라 영향 없다."""
        if self.platform == "aos":
    ...
    # 위 목록에 없지만 그 지면에 실제로 존재하는 섹션(코드가 검증 대상으로 삼지 않아 로케이터도
    # 없는 기획성 섹션)의 화면 문구. 블롭을 자를 때 경계로만 쓴다 - 하위 페이지가 오버라이드한다.
    IOS_EXTRA_END_MARKERS = []

    def _ios_end_markers(self, section_name: str, screen_text: str) -> set:
        """블롭에서 이 섹션 구간을 어디까지로 볼지 결정하는 종료 마커 집합.

        예전에는 IOS_SECTION_END_MARKERS(만화 장르홈 기준 하드코딩 목록)만 썼다. 그래서 웹툰/
        웹소설처럼 섹션 구성이 다른 지면에서는 다음 섹션 문구가 목록에 없어 구간이 끊기지 않고
        뒤쪽 섹션 여러 개를 통째로 삼켰다(2026-08-03 iOS 웹소설 "구매이력 기반 AI 추천"의 첫
        아이템에 '키워드로 작품 찾기'·'역사/시대물 베스트'·'진행중인'까지 붙어 나왔다).
        이제 그 목록에 **같은 페이지의 다른 섹션 화면 문구 전부**와 IOS_EXTRA_END_MARKERS를
        더해 자동으로 경계를 잡는다. 자기 자신은 당연히 제외한다."""
        ...
        # 자기 자신(코드 키/화면 문구)과, 자기 문구를 포함하는 마커는 즉시 0에서 잘리므로 뺀다.
    ...
    def click_section_more(self, section_name: str, more_y_nudge: float = 0.0) -> bool:
        ...
        more_y_nudge: iOS 더보기 탭 y좌표에 더할 비율 보정값(재시도용, 기본 0.0 = 보정 없음).
        스크롤 도달 지점이 실행마다 30px 정도 흔들려 고정 좌표가 빗나가는 것을 흡수한다.
        기본값이 0이라 첫 시도와 다른 모든 호출부는 기존과 동일하게 동작한다(자세한 근거는
        click_section_more_and_verify의 재시도 보정 주석 참고)."""
        if self.platform == "ios":
            ...
            # 등록 좌표는 측정 시점의 화면 기준이라, 실행 시점에 개인화 콘텐츠 길이가 달라지면
            # 섹션 위치가 30px 이상 밀려 빗나간다. 더보기 x는 화면 오른쪽 끝(0.913)이고 그 바로
            # 아래가 3열 그리드의 오른쪽 열 표지라, y가 조금만 아래로 밀려도 표지가 눌린다
            # (2026-08-03 웹소설 "내 취향 추천 신작" - 자기 섹션 3번째 작품 '비좀 포칼아립스'
            #  작품홈으로 진입). 그래서 탭 직전에 화면을 OCR해 섹션 타이틀 행을 찾고 그 y로
            # 보정한다. 못 찾으면 등록 좌표를 그대로 쓰므로 기존 동작이 유지된다.
    ...
    def click_section_more_and_verify(self, section_name: str, max_attempts: int = 3) -> tuple:
        ...
        for attempt in range(max_attempts if hint else 1):
            # 재시도 회차마다 탭 y좌표를 미세하게 흔들어 스크롤 도달 오차를 흡수한다.
            #
            # 보정 폭을 두 번 고쳤다. 처음엔 "스와이프 1회 추가"였는데 1회가 논리 200px
            # (화면 23.7%)이라 섹션을 지나쳐버렸다. 다음엔 y ±13px로 줄였는데 그것도 컸다 —
            # 실기기에서 이 섹션의 유효 구간이 **14px뿐**임이 확인됐다(2026-08-03:
            # y 388은 아래 표지가 눌려 작품홈으로, y 374는 아무것도 안 눌림).
            # 그래서 ±5px(비율 0.006)로 좁혔고, 위쪽은 미탭 경계가 가까우므로 **아래쪽을 먼저**
            # 시도한다. 첫 시도(attempt=0)는 보정이 0이라 지금 통과하는 섹션은 영향이 없다.
            ...
            # 화면이 아예 전환되지 않은 경우와 "엉뚱한 화면으로 갔다"를 구분해 남긴다.
            # iOS 목적지 화면에는 타이틀 요소가 없어 직전 화면(장르홈)의 같은 위치 요소가 대신
            # 잡히는데, 탭이 빗나가 장르홈에 그대로 있으면 상단 블롭(장르탭+서브탭) 전체가
            # "실제타이틀"로 찍힌다. 그러면 로그만 보고 오탭 목적지를 추적하려다 헛돌게 된다
            # (2026-08-03 웹소설 - '만화 만화 웹툰 웹툰 웹소설 ... 판타지 BL'이 타이틀로 찍힘).
            ...
            time.sleep(1)
        # 마지막 시도까지 실패했을 때, 읽어온 값이 실은 장르홈 상단 블롭이면 빈 문자열로 돌려준다.
        # 호출측(각 테스트 모듈)은 `navigated = bool(dest_title.strip())`으로 화면전환 여부를
        # 판단하는데, 장르홈에 머물러 있는데도 블롭이 잡혀 "화면전환 ✅"로 찍히던 문제를 여기서
        # 한 번에 막는다(2026-08-03 - 테스트 파일을 각각 고치지 않아도 네 모듈 모두 교정된다).
        if self._looks_like_genrehome_top_blob(dest_title):
            return "", False
        return dest_title, False

    # 장르홈 최상단 블롭임을 알아보는 표식(장르탭 5개가 한 문자열에 이어붙는다).
    ...
    # iOS 더보기 탭 y를 탭 직전 화면 OCR로 보정할지 여부. 만화 장르홈은 현재 등록 좌표로
    # 정상 동작하고 있어(실기기 확인) 기본값을 False로 둬 동작을 전혀 바꾸지 않는다.
    # 웹툰/웹소설 페이지 클래스에서 True로 켠다 - 그쪽은 개인화 섹션이 많아 실행마다 섹션
    # 위치가 밀리고, 더보기 x(0.913) 바로 아래가 3열 그리드 오른쪽 열 표지라 오탭 위험이 크다.
    IOS_MORE_OCR_CORRECTION = False

    # 보정으로 허용하는 최대 이동 폭(화면비율). 엉뚱한 행을 잡았을 때 크게 튀는 것을 막는다.
    # 0.25 → 0.35로 넓혔다(2026-08-03). 섹션이 화면 위로 밀린 실행에서 **진짜 더보기 후보가
    # 이 가드에 걸려 버려지고** 대신 아래쪽 아이템 행이 채택돼 오탭했다(웹소설 "구매이력 기반
    # AI 추천": 후보 y=152.2/170.2가 "0.283/0.261 벗어나 무시"된 뒤 0.526을 눌러 작품홈 진입).
    # 아래 IOS_MORE_BAND_X_CENTER 조건이 아이템 행을 확실히 걸러주므로 넓혀도 안전하다.
    IOS_MORE_OCR_MAX_SHIFT = 0.35

    # 더보기 탭이 안전한 화면 구간(상단 고정 헤더 아래 ~ 하단 네비바 위).
    IOS_MORE_ROW_SAFE_MIN = 0.22
    IOS_MORE_ROW_SAFE_MAX = 0.72

    # 타이틀 행이 안전 구간에 없을 때 위로 되돌리며 재탐색하는 최대 횟수(사용자 지시: 2회).
    IOS_MORE_ROW_RESCAN_MAX = 2

    def _ios_scroll_back_small(self):
        """화면을 위로 조금(약 110px) 되돌린다 - 지나쳐버린 섹션 타이틀을 다시 보이게 한다.

        결정론적 스크롤의 1스텝(약 180px)보다 작게 움직여, 되돌리다 앞 섹션까지 넘어가는 것을
        막는다. 되돌린 뒤에는 기억된 스크롤 위치가 실제와 달라지므로 상태를 무효화해, 다음
        섹션이 증분 스크롤을 신뢰하지 않고 최상단 리셋 경로를 타게 한다."""
        size = self.driver.get_window_size()
        x = int(size["width"] * 0.5)
        h = size["height"]
    ...
    def _ios_locate_more_row(self, section_name: str, fallback_ratio: float):
        """더보기를 누를 y(논리 px)를 화면에서 확정한다. 실패하면 None(등록 좌표 사용).

        고정 swipe 횟수는 개인화 콘텐츠 길이에 따라 어긋나, 섹션 타이틀과 더보기가 화면 위로
        아예 벗어나 있는 경우가 있다(2026-08-03 웹소설 "내 취향 추천 신작" - 그 상태에서 등록
        좌표를 누르니 자기 섹션 3번째 작품 표지가 눌렸고, 재시도 ±5px도 전부 같은 결과였다).
        화면에 없으면 OCR도 무의미하므로, 안전 구간에 들어올 때까지 위로 조금씩 되돌린다."""
        h = self.driver.get_window_size()["height"]
        ...
        for attempt in range(self.IOS_MORE_ROW_RESCAN_MAX + 1):
            ...
            rescanned = True
        # 확정 실패 시 등록 좌표로 폴백하는데, 되돌림을 한 번이라도 했다면 화면이 그만큼
        # 아래로 밀려 있어 등록 좌표(되돌리지 않은 화면 기준으로 측정한 값)가 완전히 빗나간다.
        # 실제로 이 복구를 넣기 전에는 "내 취향 추천 신작"이 3회 시도 전부 아무것도 못 누르고
        # 실패했다(2026-08-03 - 되돌림 220px 상태에서 등록 y를 눌러 빈 영역을 탭).
        # 그래서 폴백 전에 결정론적 스크롤로 기준 위치를 되돌려놓는다.
    ...
    # 더보기 텍스트 띠 판별 기준(밝기 임계 픽셀 스캔). 실측 서명은 모든 장르홈/탭에서 일정하다:
    #   더보기 텍스트  두께 7~14px, x중심 356(=0.913), 회색(밝기<170)
    #   섹션 타이틀    두께 10~24px, 굵은 검정(밝기<110), 더보기와 같은 선상(중심 차 ≤12px)
    ...
    # 더보기 텍스트의 x중심(논리 px)과 허용 오차. 오늘 실측한 14개 좌표가 **전부 356.0**이었다
    # (추천/로맨스/BL/판타지 탭 + 웹소설). 이 조건이 없으면 3열 그리드 아이템 행이 더보기로
    # 오인된다 - 좌측은 작품명(굵은 검정), 우측은 별점 "4.9 (1,635)"(회색)이라 두께·색 조건을
    # 그대로 만족하기 때문이다. 실제로 웹소설 "내 취향 추천 신작"에서 아이템 행(0.412)을 잡아
    # 3번째 작품 작품홈으로 3회 연속 오탭했다(2026-08-03). 별점은 x가 유동적이라 이 조건으로
    # 확실히 걸러진다.
    IOS_MORE_BAND_X_CENTER = 356.0
    IOS_MORE_BAND_X_TOL    = 6.0
    # x중심을 구할 때 쓰는 넓은 스캔 창. 좁은 창(IOS_MORE_BAND_X)으로 구하면 어떤 텍스트든
    # 창 중앙값이 나와 판별이 무력해진다(band_x_center 주석 참고).
    IOS_MORE_XSCAN_X       = (0.55, 1.00)

    # 섹션명 대조용 좌측 타이틀 크롭 파라미터(2026-08-04 조합 실측에서 가장 안정적이던 값).
    ...
    # x 덩어리를 끊는 최소 여백(논리 px). 더보기는 앞의 타이틀과 이 이상 떨어져 있다.
    IOS_MORE_CLUSTER_GAP = 15

    def _ios_title_row_y_from_screen(self, section_name: str, fallback_ratio: float):
        """화면에서 더보기가 있는 행의 y중심(논리 px)을 찾아 반환한다. 못 찾으면 None.

        iOS는 섹션 타이틀도 더보기도 개별 접근성 요소로 존재하지 않아(2026-08-03 실기기 재확인:
        name == "더보기" / name == "{섹션명}" 모두 0개, 블롭 컨테이너만 CONTAINS로 잡힘) 좌표
        탭이 유일한 수단이고, 위치는 화면 픽셀에서 읽어야 한다.

        처음에는 pytesseract로 섹션명을 찾았는데 **굵은 섹션 타이틀 행을 통째로 못 읽는 사례가
        반복**됐다(2026-08-03: 웹툰 "새로나온작품", 웹소설 "내 취향 추천 신작" - 화면에 또렷이
        보이고 위아래 행은 읽히는데 그 행만 누락). 그래서 문자 인식을 버리고 밝기 임계로
        텍스트 띠(band)를 직접 찾는다 - 오늘 14개 좌표를 이 방식으로 실측해 전부 맞았다.

        판별은 "우측 회색 띠(더보기) + 같은 선상 좌측 굵은 띠(섹션 타이틀)" 쌍으로 한다.
        표지/평점 등은 좌측 굵은 띠와 같은 선상에 오지 않아 자연히 걸러진다."""
        try:
            from PIL import Image
            import io
            screen_text = self.IOS_SECTION_BLOB_ANCHOR.get(section_name, section_name)
            ...
            def band_x_center(ma, mb):
                """띠 안에서 '더보기'로 보이는 텍스트 덩어리의 x중심(논리 px). 없으면 None.

                판별을 x '덩어리(cluster)' 단위로 한다. 이 방식에 두 번 도달했다.
                  1) 처음엔 좁은 창(0.83~0.99)에서 min/max 중심을 썼다 → 어떤 텍스트든 창
                     중앙(≈355)이 나와 아이템 행이 그대로 통과했다.
                  2) 창을 넓혀(0.55~1.0) min/max를 썼다 → 섹션 타이틀이 긴 경우
                     ("41q... 님의 구매이력 기반 AI 추천"은 논리 x 253까지 뻗는다) 타이틀 꼬리가
                     창에 들어와 중심이 300 부근으로 계산되고, 진짜 더보기가 탈락했다.
                덩어리로 끊으면 둘 다 해결된다 - 더보기는 앞뒤가 흰 여백으로 끊긴 독립 덩어리라
                항상 x중심 356 / 폭 30px 내외로 잡히고, 타이틀 꼬리·별점은 별개 덩어리가 된다.
                """
            ...
            def title_text_at(ma, mb):
                """후보 더보기와 같은 선상의 좌측 타이틀 영역만 크롭해 OCR한 한글 문자열.

                픽셀 방식만으로는 "더보기 텍스트다"까지만 알 수 있고 **어느 섹션의 더보기인지**는
                구분하지 못한다. 그래서 스크롤이 과해 다음 섹션의 더보기가 화면에 들어오면 그것을
                잡아버렸다(2026-08-04 "새로 나온 작품" → 'BL 역사/시대물 베스트' 더보기를 눌러
                3회 연속 실패). 전체 화면 OCR은 굵은 타이틀 행을 통째로 놓치는 사례가 있었지만,
                타이틀 한 줄만 좁게 크롭하면 인식률이 크게 올라간다.

                핵심은 --psm 7(단일 행 인식 모드)이다. 기본 psm은 같은 크롭에서도 빈 문자열이나
                엉뚱한 값('바도르')을 내는데, psm 7로 바꾸면 pad/폭/배율 조합 전반에서 안정적으로
                '내취향추천신작'을 읽었다(2026-08-04 조합 실측 - 성공 26건 중 24건이 psm 6/7).
                아래 상수는 그 실측에서 가장 안정적이던 조합이다."""
                import pytesseract
            ...
            # 섹션명을 **한글 조각 목록**으로 쪼개 비교한다. 통째로 이어붙인 문자열로 `in`
            # 비교하면 영문이 섞인 섹션명에서 실패한다 - "구매이력 기반 AI 추천"은 한글만
            # 남기면 '구매이력기반추천'이 되는데, OCR은 화면의 "AI"를 '씨'로 읽어
            # '님의구매이력기반씨추천'을 준다. 중간에 '씨'가 끼어 `in`이 깨진다(2026-08-04 실측:
            # 되돌림 3회 동안 매번 올바른 섹션을 읽었는데도 전부 "다른 섹션"으로 버렸다).
            # 조각을 모두 포함하는지로 보면 영문 오인식과 무관하게 판정된다.
            # ('BL'을 '8Ｌ'로 읽는 사례를 _korean_only로 다루던 것과 같은 이유다.)
            ...
            for ma, mb in more_bands:
                if not (self.IOS_MORE_BAND_TH[0] <= mb - ma <= self.IOS_MORE_BAND_TH[1]):
                    continue
                # 더보기는 x중심이 356으로 고정이다. 아이템 별점 텍스트는 x가 유동적이라 여기서
                # 걸러진다(이 조건이 없어 아이템 행을 잡고 작품홈으로 오탭한 이력이 있다).
                ...
                # 좌측 타이틀을 크롭 OCR해 이 섹션의 더보기인지 확인한다. 일치하면 등록값에서
                # 얼마나 떨어졌든 채택한다(섹션명이 맞으면 그게 정답이므로 거리 가드가 불필요).
                # 일치하지 않으면 후보로 두되, 섹션명이 확인된 후보가 하나도 없을 때만 쓴다.
                ...
                if best is None or shift < best[1]:
                    best = (mc, shift, tkor)
            # 섹션명이 확인된 후보가 없으면 좌표를 돌려주지 않는다(None).
            # 예전에는 "그래도 더보기 텍스트니까" 하고 폴백으로 반환했는데, 그게 **다른 섹션의
            # 더보기**여서 그대로 눌러 오탭했다(2026-08-04 "새로 나온 작품" → 'BL 역사/시대물
            # 베스트', "구매이력 기반 AI 추천" → 작품홈). None을 반환하면 호출측
            # (_ios_locate_more_row)이 위로 되돌려 재탐색하므로, 남의 섹션을 누르는 대신
            # 내 섹션이 화면에 들어올 기회를 얻는다.
            if best:
                self.log.info(
    ...
    # 푸터 감지용 하단 OCR 크롭 비율(iOS 전용). 0.15로는 화면 하단 15%(390x844에서 y 717~844)
    # 만 잘라내는데 그 구간에는 하단 네비게이션바만 들어오고, 최하단까지 내려가도 푸터 텍스트는
    # 그보다 위에 있어(실측: "리디(주) 사업자 정보" y 645.7 / 이용약관 y 671 / ©RIDI Corp. y 696)
    # 스크롤을 아무리 해도 감지되지 않았다 - max_scroll을 전부 소진한 진짜 원인이 이것이었다
    # (2026-08-03 iOS 실기기, logs/diag/footer_reached.png). 0.30이면 y 591부터 잘라 푸터가
    # 안정적으로 들어온다. 이 값은 이 경로 전용이라 _is_text_visible_on_screen의 기본값(0.12)과
    # 목적지 타이틀 확인 등 다른 호출부에는 영향이 없다.
    ...
    def scroll_to_footer_and_get_last_item(self, section_name: str, max_scroll: int = 60) -> tuple:
        ...
        iOS 실측(2026-08-03 웹툰 추천탭)에서는 섹션 도달 지점부터 푸터까지 약 35회가 필요해
        40으로도 빠듯했다 - 60으로 올려 여유를 둔다.
        ...
        불가능하다 - 화면 하단 영역을 스크린샷+OCR로 읽어 노출 여부를 확인한다(크롭 범위는
        IOS_FOOTER_OCR_BOTTOM_RATIO 주석 참고)."""
...
class WebtoonGenrePage(ComicGenrePage):
    ...
    IOS_SECTION_SWIPE_COUNT = {
        **ComicGenrePage.IOS_SECTION_SWIPE_COUNT,
        ...
        "지금리디에서만볼수있는 웹툰": 19,
        "새로나온작품":             22,
        # 25로 등록했다가 26으로 교정했다. 최초 측정은 앞 구간(0~14)을 찍은 화면에서 "이어서"
        # 15~24를 찍어 스크롤 누적 위치가 실제 실행(딥링크 최상단부터 시작)과 어긋났다.
        # 딥링크부터 26회를 끊김 없이 다시 찍어 확인한 결과 swipe25에서는 타이틀이 화면
        # 하단(y비율 0.657)이고 swipe26에서 중앙(0.439)에 온다(2026-08-02 재측정).
        "취향저격 AI추천 섹션":      26,
        # 로맨스 탭 - iOS 실기기 실측(2026-08-03). 기존 값은 "섹션 순서대로 1,2,3..."에 가까운
        # 추정값이었는데 실제와 최대 2배 이상 차이났다(키워드 검색 6 -> 13). 원인은 "실시간 랭킹"이
        # 10위까지 나열되는 세로 랭킹 리스트라 그 다음 섹션까지 거리가 멀고, 그 오차가 뒤로 가며
        # 누적되기 때문이다. 딥링크부터 끊김 없이 22프레임을 촬영해 각 섹션 타이틀이 화면 중앙
        # 부근(y비율 0.25~0.70)에 오는 지점을 채택했다(logs/diag/romance_frames/).
        # 참고: 이 탭의 최하단은 swipe 19에서 도달한다(19~22 프레임이 동일).
        "오늘, 리디의 발견":        3,
        "실시간 랭킹":             5,
        "로맨스 기다리면 무료!":     8,
        "로맨스 베스트":            9,
        "웹툰/만화 키워드 검색":    13,
        "오직 리디에서만!":        15,
        # BL 탭 섹션들 - iOS 실기기 실측(2026-08-03, 30프레임 연속 촬영 / logs/diag/frames_BL/).
        # 기존 추정값(2~10)은 실제와 최대 2배 이상 어긋났다(이 작품 어때요 10 -> 21). 로맨스 탭과
        # 같은 이유다 - "BL웹툰 실시간 랭킹"이 10위까지 나열되는 세로 랭킹 리스트라 다음 섹션까지
        # 거리가 멀고 그 오차가 뒤로 가며 누적된다. 게다가 iOS 화면에는 코드에 없는 섹션이 3개 더
        # 끼어 있어("지금 오직 리디" swipe10~11 / "비욘드 BL웹툰 모아보기!" swipe17 /
        # "8월의 리다무 파티!" swipe18~19) 그만큼 뒤 섹션이 밀린다.
        # 또 AOS와 순서가 다르다 - AOS는 요일별 웹툰 다음이 "지금, 리디에서만~"인데 iOS는
        # 키워드 검색 -> "지금 오직 리디" -> 요일별 웹툰 -> "지금, 리디에서만~" 순이다.
        "BL웹툰 실시간 랭킹":       2,
        "BL 오늘, 리디의 발견":     4,
        # 이 계정에서는 개인화 미노출이라 실측하지 못했다(AOS BL 로그에서도 조회 반복 타임아웃).
        # "BL 오늘, 리디의 발견"(4) 바로 다음이라는 AOS 순서만 근거로 둔 값이다. 노출되는 실행이
        # 나오면 확인이 필요하고, 노출되면 이후 섹션 전부가 그만큼 밀린다.
        "BL 구매이력기반 AI 추천":  5,
        "BL웹툰 베스트":           7,
        "BL키워드 검색":           9,
        "BL 요일별 웹툰":         12,
        ...
        # 판타지/SF 탭 - iOS 실기기 실측(2026-08-03, 28프레임 연속 촬영 /
        # logs/diag/frames_판타지_SF/). 앞 두 개(2,3)는 추정값이 맞았고 뒤로 갈수록 어긋났다
        # (새로나온작품 8 -> 22). AOS와 순서도 다르다 - AOS는 "RIDI ONLY 판타지" 다음이
        # "오직 리디에서만!"인데 iOS는 반대다(베스트 -> RIDI ONLY 판타지 -> 오직 리디에서만!).
        # "이 판타지 어때요?"(11)와 "판타지 새로나온작품"(22) 사이에는 코드에 없는 긴 그리드
        # 구간이 있어 10회 이상 벌어진다.
        "판타지 오늘, 리디의 발견": 2,
        "판타지 기다리면 무료!":   3,
        "판타지 베스트":         5,
    ...
    IOS_SECTION_MORE_COORD_RATIO = {
        ...
        "웹툰 베스트":              (0.910, 0.472),
        # 실행과 동일한 조건(스와이프 간 대기 1.3초)으로 19회 스크롤한 화면을 픽셀 단위로 실측한
        # 결과다(2026-08-03, logs/diag/more_btn_swipe19.png):
        #     위쪽 배너 하단  y 323.3
        #     더보기 텍스트   y 373.0 ~ 385.3  (두께 12.3)
        #     작품 표지 상단  y 401.7
        # 실기기에서 두 지점을 눌러 유효 범위를 좁혔다(2026-08-03 사용자 확인):
        #     y 388 (0.460) → 아래 작품 표지가 눌려 작품홈으로 진입
        #     y 374 (0.444) → **아무것도 안 눌림**(화면 전환 없음). 텍스트 상단에 너무 붙어
        #                     버튼의 실제 터치 영역을 벗어났다.
        # 즉 유효 구간은 374 < y < 388(14px)뿐이다. 그 중간(381)에서 살짝 위인 텍스트 중심
        # y=379를 쓴다 - 표지까지 22px, 미탭 경계까지 5px 여유.
        "지금리디에서만볼수있는 웹툰": (0.910, 0.449),
        # 같은 방식으로 실측·검증했다(2026-08-03, logs/diag/more_btn_swipe22.png):
        #     더보기 텍스트   y 434.3 ~ 443.3  (두께 9.0, x중심 356.0)
        #     작품 표지 상단  y 461.7
        # 기존 등록값 0.536(y 452.4)은 텍스트 하단보다 9px 아래의 빈 공간이라, 위 섹션에서
        # 확인된 실패 양상(텍스트 위 = 미탭 / 아래 = 표지 오탭) 그대로 오탭 위험 구간이었다.
        # 텍스트 중심 y 439를 실기기에서 눌러 "새로 나온 작품" 목적지 진입을 확인했다.
        # 이 섹션은 OCR(pytesseract)이 타이틀 행과 '더보기'를 통째로 놓쳐(아이템 제목은 읽음)
        # 밝기 임계로 텍스트 띠를 직접 찾아 실측했다 - logs/diag/ios_pixel_row_scan.py.
        "새로나온작품":             (0.913, 0.520),
        # 로맨스 탭 - iOS 실기기 실측(2026-08-03). 위 IOS_SECTION_SWIPE_COUNT를 실측값으로
        # 고친 뒤 각 지점 프레임을 밝기 임계로 스캔해 더보기 텍스트 띠를 직접 쟀다. 네 섹션 모두
        # 두께 9.0px / x중심 356.0으로 일정하고 좌측 섹션 타이틀과 같은 선상이다(추천 탭 실측과
        # 동일한 값 - 웹툰 장르홈의 더보기는 항상 같은 우측 위치다).
        # 기존 x 0.932(=y 363.5)는 더보기 텍스트 우측 끝에 가깝고 y는 전부 빗나간 추정값이었다.
        "실시간 랭킹":             (0.913, 0.455),
        "로맨스 기다리면 무료!":     (0.913, 0.279),
        "로맨스 베스트":            (0.913, 0.458),
        "웹툰/만화 키워드 검색":     (0.913, 0.457),
        # BL 탭 섹션들 - iOS 실기기 실측(2026-08-03). 위 IOS_SECTION_SWIPE_COUNT를 실측값으로
        # 고친 뒤 각 지점 프레임을 밝기 임계로 스캔해 더보기 텍스트 띠를 직접 쟀다. 여섯 섹션 모두
        # 두께 9.0px / x중심 356.0(=0.913)으로 일정하고 좌측 섹션 타이틀과 같은 선상이다 -
        # 추천/로맨스 탭 실측과 동일한 x다(웹툰 장르홈의 더보기는 항상 같은 우측 위치).
        "BL웹툰 실시간 랭킹":       (0.913, 0.365),
        # 개인화 미노출로 실측 불가 - 기존 추정값을 그대로 둔다(위 swipe 주석 참고).
        ...
        # 판타지/SF 탭 - iOS 실기기 실측(2026-08-03). 위 IOS_SECTION_SWIPE_COUNT를 실측값으로
        # 고친 뒤 각 지점 프레임을 밝기 임계로 스캔했다. 네 섹션 모두 두께 9.0px / x중심
        # 356.0(=0.913)으로 추천·로맨스·BL 탭과 동일하다. "이 판타지 어때요?"는 더보기가 없어
        # (2026-08-03 사용자 확인 + 실기기 스크린샷) 의도적으로 등록하지 않는다.
    ...
    # 더보기 탭 y를 탭 직전 화면 OCR로 보정한다(ComicGenrePage 기본값 False를 덮어씀).
    # 이 장르홈은 개인화 섹션이 많아 실행마다 섹션 위치가 30px 이상 밀리고, 더보기 x(0.913)
    # 바로 아래가 3열 그리드 오른쪽 열 표지라 조금만 밀려도 작품홈으로 오탭된다.
    ...
    IOS_SECTION_BLOB_ANCHOR = {
        ...
        # 로맨스/BL/판타지 탭 (2026-08-03 사용자 확인).
        # "BL "/"판타지 " 접두사는 탭별로 같은 문구를 구분하기 위한 **코드 전용 키**이고 화면에는
        # 없다. 이 사전에 넣지 않으면 블롭에서 접두사까지 붙은 문자열을 찾다 실패한다.
        # 접두사가 붙었는데도 화면 문구와 같은 것("판타지 기다리면 무료!"/"판타지 베스트"/
        # "BL웹툰 ~"/"BL키워드 검색")은 화면에 그대로 나오므로 등록하지 않는다.
        "BL 오늘, 리디의 발견":            "오늘, 리디의 발견",
        "BL 요일별 웹툰":                 "요일별 웹툰",
        # ⚠️ 이 항목만 실측이 아니라 추론이다. "BL 구매이력기반 AI 추천"은 개인화 섹션이라
        # AOS BL 실행에서 미노출이었고(로그에서 textContains("구매이력") 조회가 반복 타임아웃)
        # 화면 문구를 직접 확인할 기회가 없었다. 추천 탭의 동일 섹션은 실측으로 화면/목적지가
        # 모두 띄어쓰기("42q... 님의 구매이력 기반 AI 추천")임이 확인됐고 같은 컴포넌트로
        # 보이므로 그 표기를 따른다. 코드 키는 붙여쓰기 + "BL " 접두사라 매핑이 없으면 블롭에서
        # 확실히 못 찾는다. iOS 실기기에서 이 섹션이 노출되는 실행이 나오면 확인이 필요하다.
...
class WebnovelGenrePage(ComicGenrePage):
    ...
    # iOS 실기기 실측(2026-08-03, 24프레임 연속 촬영 / logs/diag/frames_webnovel_추천/).
    # 기존 1~7 순차 추정값은 실제와 크게 어긋났다(진행중인 이벤트 6 -> 18). "웹소설 실시간 랭킹"이
    # 세로 랭킹 리스트라 다음 섹션까지 멀고, 게다가 코드에 없는 iOS 전용 섹션이 3개 더 끼어 있다
    # ("BL 현대물 베스트" swipe9~10 / "키워드로 작품 찾기" swipe13~14 /
    #  "역사/시대물 베스트" swipe15~17). 각 섹션 타이틀이 화면 중앙 부근에 오는 지점을 채택했다.
    IOS_SECTION_SWIPE_COUNT = {
        "방금 본 작품과 비슷한":  2,
        "내 취향 추천 신작":     5,
        "웹소설 실시간 랭킹":    6,
    ...
    # iOS 실기기 실측(2026-08-03). 위 IOS_SECTION_SWIPE_COUNT를 실측값으로 고친 뒤 각 지점
    # 프레임을 밝기 임계로 스캔해 더보기 텍스트 띠를 직접 쟀다. 네 섹션 모두 두께 9.0px /
    # x중심 356.0(=0.913)으로, 웹툰 장르홈 4개 탭에서 나온 값과 동일하다.
    IOS_SECTION_MORE_COORD_RATIO = {
    ...
    # 코드 키와 실제 화면 문구가 다른 섹션(iOS 블롭 파싱용). WebtoonGenrePage의 동명 사전과
    # 같은 목적이다 - 로케이터는 화면 문구로 블롭 요소를 찾지만, 블롭 **안에서** 코드 키를
    # 찾으면 없어서 콘텐츠를 못 읽는다. 아래 두 건은 2026-08-03 실기기 화면으로 확인했다.
    #   취향저격 AI추천 섹션 → "41q... 님의 취향 저격 AI 추천"(계정ID 접두사가 붙어 고정 접미사로 매칭)
    # "방금 본 작품과 비슷한"은 화면에도 그 문구 그대로 나오므로(2026-08-03 실기기 확인) 등록하지
    # 않는다. 프레임 OCR에서 "<제비 사냥>와 비슷한"으로 읽힌 행은 이 섹션이 아니라 그 위의 다른
    # 섹션이었다 - OCR 판독만으로 화면 문구를 단정하면 안 된다는 사례로 남긴다.
    IOS_SECTION_BLOB_ANCHOR = {
        "취향저격 AI추천 섹션":   "님의 취향 저격 AI 추천",
    }

    # 웹툰과 동일한 이유로 더보기 y를 탭 직전 OCR로 보정한다 - 실제로 "내 취향 추천 신작"에서
    # 등록 좌표(y 304)가 실행 시점엔 표지 안쪽이 되어 자기 섹션 3번째 작품('비좀 포칼아립스')
    # 작품홈으로 진입했다(2026-08-03 iOS 실기기).
    IOS_MORE_OCR_CORRECTION = True

    # 코드가 검증 대상으로 삼지 않는 iOS 화면 실존 섹션들(2026-08-03 프레임 판독).
    # 블롭 구간을 자를 경계로만 쓴다 - 이게 없으면 "구매이력 기반 AI 추천" 구간이 뒤쪽 섹션
    # 3개를 통째로 삼켰다.
    IOS_EXTRA_END_MARKERS = [
...
class GeneralbookGenrePage(ComicGenrePage):
    ...
    # ── 도서 장르홈 추천탭 (2026-08-03 AOS 에뮬레이터 실측) ─────────────────────────
    # 계정 4qatest로 로그인해 페이지소스를 직접 읽어 노출 순서/더보기 유무를 확정했다
    # (logs/diag/aos_general_relogin2.out). 실측 순서는 아래와 같고, 검증 대상이 아닌
    # 기획성 섹션 4개는 등록하지 않는다:
    #   방금 본 작품과 비슷한 → 지금 많이 읽고 있는 작품 → 오늘, 리디의 발견
    #   → 구매이력 기반 AI 추천 → 이벤트 → 베스트
    #   → (<마션 (스페셜 에디션)>과 비슷한 / 지금, 리디에서만! 선 출간 신작)
    #   → 새로 나온 작품 → (히가시노 게이고 작가 대표작 / 짧지만 강렬한 서사, 우주라이크소설)
    #   → 지금, 리디에서만 볼 수 있는 도서 → 취향 저격 AI 추천(최하단, 푸터 존재)
    QUICK_MENU_LOCATOR = {
        "신간":       "NEW_QUICK",
        "북스 베스트": "BEST_QUICK",
        "이벤트":     "EVENT_QUICK",
    ...
    # 퀵메뉴 선택 후 목적지 타이틀(2026-08-03 실기기 실측값 기준).
    #   신간       → '일반도서 신간'      이라 고정부 '신간'으로 매칭
    #   북스 베스트 → '북스 베스트'
    #   이벤트     → '이벤트'
    #   리디온리   → '[5% 포인트백] 당신에게 가장 먼저🎁 RIDI ONLY 도서' 처럼 프로모션 문구가
    #                앞에 붙고 뒤에 지면명이 온다. 처음엔 기대값을 '리디온리'로 뒀다가 실패했다
    #                (메뉴 라벨과 목적지 지면명이 다르다) - 고정부 'RIDI ONLY 도서'로 매칭한다.
    QUICK_MENU_EXPECTED_TITLE = {
        "신간":       "신간",
        "북스 베스트": "베스트",
        "이벤트":     "이벤트",
    ...
    # 더보기가 있는 섹션의 목적지 타이틀(사용자 지정). 더보기 없는 섹션은 등록하지 않는다 -
    # 이 사전은 _get_ios_section_content가 앵커를 "{섹션명} 더보기"로 잡을지 판단하는 기준으로도
    # 쓰여, 더보기 없는 섹션을 넣으면 콘텐츠 추출이 깨진다.
    IOS_SECTION_MORE_DEST_HINT = {
        "지금 많이 읽고 있는 작품":          "지금 많이 읽고 있는 작품",
        # 목적지 타이틀에 계정ID 접두사가 붙으므로("4qa... 님의 구매이력 기반 AI 추천") 고정
        # 접미사로 매칭한다 - 웹툰/웹소설의 동일 섹션과 같은 방식이다.
        "구매이력 기반 AI 추천":            "구매이력 기반 AI 추천",
        "이벤트":                        "이벤트",
        "베스트":                        "베스트",
        "새로 나온 작품":                  "새로 나온 작품",
    ...
    # 개인화 섹션 - 계정 상태에 따라 노출이 갈린다. 실측에서 43qatest는 구매이력이 없어
    # "구매이력 기반 AI 추천"과 "취향저격 AI추천 섹션"이 둘 다 미노출이었고, 4qatest는 둘 다
    # 노출됐다(2026-08-03). 미노출이면 하드 실패 대신 스킵한다.
    AOS_PERSONALIZED_SECTIONS = ComicGenrePage.AOS_PERSONALIZED_SECTIONS | {
        "지금 많이 읽고 있는 작품",
    }

    # 순위형(1·2·3… 번호가 붙는) 섹션. 아이템 분리 규칙이 일반 그리드와 다르다.
    RANKED_SECTIONS = {"지금 많이 읽고 있는 작품"}

    # ── iOS 도서 장르홈 (2026-08-04 실기기 실측, 로그인 상태 / QA iPhone 16e 390x844) ──────
    # 이 지면도 블롭 구조라 결정론적 스와이프 + 블롭 파싱 + 좌표 탭 방식을 쓴다.
    # 22프레임을 딥링크부터 끊김 없이 촬영해(logs/diag/frames_generalbook/) 각 섹션 타이틀이
    # 화면 중앙 부근에 오는 지점을 잡고, 그 프레임을 밝기 임계로 스캔해 더보기 좌표를 실측했다.
    # 실측 순서(AOS와 다르다 - AOS는 "방금 본 작품과 비슷한"이 맨 앞):
    #   지금 많이 읽고 있는 작품(2) → 오늘, 리디의 발견(4) → 구매이력 기반 AI 추천(7)
    #   → <작품명>와 비슷한(8) → 이벤트(9) → 베스트(11)
    #   → (지금, 리디에서만! 선 출간 신작 15 / 히가시노 게이고 작가 대표작 18 /
    #      짧지만 강렬한 서사, 우주라이크소설 21 - 검증 대상 아님)
    #   → 새로 나온 작품(17) → 지금, 리디에서만 볼 수 있는 도서(22)
    IOS_SECTION_SWIPE_COUNT = {
        "지금 많이 읽고 있는 작품":          2,
        "오늘, 리디의 발견":               4,
        "구매이력 기반 AI 추천":            7,
    ...
    # 더보기 좌표. 실측 6개 모두 x중심 356.0(=0.913) / 두께 9~10px으로, 만화·웹툰·웹소설과
    # 동일한 서명이다. 더보기 없는 섹션(방금 본 작품과 비슷한 / 오늘, 리디의 발견 /
    # 취향저격 AI추천 섹션)은 등록하지 않는다 - 이 사전은 _get_ios_section_content가 앵커를
    # "{섹션명} 더보기"로 잡을지 판단하는 기준으로도 쓰여, 넣으면 콘텐츠 추출이 깨진다.
    IOS_SECTION_MORE_COORD_RATIO = {
    ...
    # 코드 키와 실제 화면 문구가 다른 섹션(iOS 블롭 파싱용).
    #   구매이력 기반 AI 추천 → "4qa... 님의 구매이력 기반 AI 추천"(계정ID 접두사) - 고정 접미사로 매칭
    #   취향저격 AI추천 섹션  → "님의 취향 저격 AI 추천"
    #   방금 본 작품과 비슷한 → iOS는 "<프로젝트 헤일메리>와 비슷한"처럼 직전 열람 작품명이 들어가
    #     매번 바뀐다(AOS는 고정 문구라 이 차이가 있다). 고정부인 "와 비슷한"만 앵커로 쓴다.
    IOS_SECTION_BLOB_ANCHOR = {
        "취향저격 AI추천 섹션":  "님의 취향 저격 AI 추천",
        "방금 본 작품과 비슷한": "와 비슷한",
    }

    # 코드가 검증 대상으로 삼지 않는 iOS 화면 실존 섹션들. 블롭 구간을 자를 경계로만 쓴다.
    IOS_EXTRA_END_MARKERS = [
        "지금, 리디에서만! 선 출간 신작", "히가시노 게이고 작가 대표작",
        "짧지만 강렬한 서사, 우주라이크소설",
    ]

    # 웹툰/웹소설과 동일한 이유로 더보기 y를 탭 직전 화면에서 보정한다(개인화 섹션이 많아
    # 실행마다 위치가 밀리고, 더보기 x 바로 아래가 아이템이라 오탭 위험이 크다).
    IOS_MORE_OCR_CORRECTION = True

    # 빅배너 영역의 y비율 범위(1080x2340 실측 y 371~1401 기준). 인디케이터 숫자를 이 안에서만
    # 찾아 서브탭 숫자·평점 등 다른 숫자에 속지 않게 한다.
    BIG_BANNER_Y_RATIO = (0.14, 0.62)

    def get_big_banner_page_indicator(self) -> tuple:
        """빅배너 페이지 인디케이터 (현재, 전체)를 반환한다. 못 읽으면 (None, None).

        도서 장르홈 빅배너는 다른 장르홈과 달리 **배너 문구 텍스트가 없다**(2026-08-03 실기기
        실측 - 배너 영역에 TextView가 인디케이터 숫자뿐). 그래서 문구를 모으는
        collect_big_banner_items_by_polling 방식이 이 지면에는 성립하지 않아, 대신 배너 우상단
        인디케이터("5 8" = 5/8)의 현재 페이지 번호로 배너 전환을 확인한다.
        """
        if self.platform != "aos":
            return (None, None)
        import re as _re
    ...
    def collect_big_banner_pages_by_polling(self, target_count: int = 5,
        ...
        """빅배너가 자동 전환되는 동안 인디케이터의 현재 페이지 번호를 모아 반환한다.

        collect_big_banner_items_by_polling(문구 수집)의 도서 장르홈 대응판이다 - 같은 목적
        ("서로 다른 배너 N개가 실제로 전환되는지")을 문구 대신 페이지 번호로 확인한다."""
        seen, order = set(), []
