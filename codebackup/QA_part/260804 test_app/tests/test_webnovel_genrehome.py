...
class TestGenrehome:
    ...
    def test_003_서브탭_순차이동_정역방향(self):
        ...
        # 정방향: 추천 → 로맨스 → 로판 → BL → 판타지
        for tab in ["로맨스", "로판", "BL", "판타지"]:
        ...
        # 역방향: 판타지 → BL → 로판 → 로맨스 → 추천
        for tab in ["BL", "로판", "로맨스", "추천"]:
            self.page.click_subtab(tab, log=False)
            time.sleep(1)
            ok = self.page.is_subtab_visible(tab, log=False)
            logging.info(f"[서브탭 선택이동확인] {tab} {'✅' if ok else '❌'}")
        ...
        assert self.page.is_quickmenu_visible("신작"), \
            "❌ 추천 서브탭 복귀 후 신작 퀵메뉴 미노출"
...
class TestRecommendtab:
    ...
    def _run_section_flow(self, section_name: str, click_recommend_first: bool = False, on_more_screen=None,
        ...
        if not verified:
            ...
            back_ok = self.page.is_webnovel_genrehome_displayed()
            pytest.fail(
    ...
    def test_007_진행중인이벤트_섹션(self):
        section_name = "진행중인 이벤트"
        self._run_section_flow(section_name, on_more_screen=lambda: self._log_more_screen_first_last(section_name))

    def test_008_취향저격AI추천_섹션(self):
        if not self.page.is_webnovel_genrehome_displayed():
