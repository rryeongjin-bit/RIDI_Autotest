...
class AOS_WEBTOON_GENRE:
    ...
    # BL탭 전용. 위 SECTION_KEYWORD_SEARCH("키워드 검색" 부분일치)는 추천탭의 "웹툰/만화 키워드
    # 검색"에도 걸려서, BL탭에서 엉뚱한 섹션을 잡았다(2026-08-04 iOS 로그 test_007 실패).
    SECTION_BL_KEYWORD_SEARCH = (AppiumBy.ANDROID_UIAUTOMATOR, 'new UiSelector().text("BL 키워드 검색")')
...
class IOS_WEBTOON_GENRE:
    ...
    # BL탭 전용. 위 SECTION_KEYWORD_SEARCH("키워드 검색" 부분일치)는 추천탭의 "웹툰/만화 키워드
    # 검색"에도 걸려서, BL탭에서 엉뚱한 섹션의 블롭을 잡았다(2026-08-04 iOS 로그 test_007 실패 -
    # 150초 대기 후 "앵커를 블롭에서 찾지 못함"). 만화(IOS_COMIC_GENRE)에는 이미 있던 로케이터다.
    SECTION_BL_KEYWORD_SEARCH = (AppiumBy.IOS_CLASS_CHAIN, '**/XCUIElementTypeOther[`name CONTAINS "BL 키워드 검색"`]')
