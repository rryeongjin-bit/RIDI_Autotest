...
TEST_MODULES = [
    ...
    {"value": "tests/test_comic_genrehome.py", "label": "test_comic_genrehome.py"},
    {"value": "tests/test_genre_category.py", "label": "test_genre_category.py"},
    {"value": "tests/test_basic_genrehome.py", "label": "test_basic_genrehome.py"},
    {"value": "tests/test_webtoon_genrehome.py", "label": "test_webtoon_genrehome.py"},
    {"value": "tests/test_webnovel_genrehome.py", "label": "test_webnovel_genrehome.py"},
    {"value": "tests/test_general_genrehome.py", "label": "test_general_genrehome.py"},
]
...
def interactions():
    logger.info("[interactions] 요청 도달")
    if not verify_slack_signature(request):
        logger.warning("[interactions] 서명 검증 실패 → 403")
        return jsonify({"error": "invalid signature"}), 403

    payload = json.loads(request.form.get("payload", "{}"))
    logger.info(f"[interactions] type={payload.get('type')}, callback_id={payload.get('callback_id')}")

    # 바로가기(Shortcut) 클릭/선택 → 즉시 모달 오픈 (전송 불필요)
    if payload.get("type") in ("shortcut", "message_action"):
        cb = payload.get("callback_id")
        trigger_id = payload.get("trigger_id")
        if cb == "qa-apptest-start":
            _open_start_modal(trigger_id)
        elif cb == "qa-apptest-stop":
    ...
    try:
        ...
        if parallel:
            for p in ["aos", "ios"]:
                ...
                thread_resp = _post_thread(channel, ts, f"<@{user_id}> 님이 {p.upper()} 테스트를 시작했습니다.")
            ...
            thread_resp = _post_thread(channel, ts, f"<@{user_id}> 님이 테스트를 시작했습니다.")
            thread_ts   = thread_resp["ts"]
            with _runs_lock:
                _active_runs[run_id] = {
                    "channel":     channel,
...
def _execute_tests(run_id, platform, modules, env_selected, env_label, parallel, channel):
    ...
    def on_log(platform_key: str, log_chunk: str):
    ...
    def on_finish(platform_key: str, returncode: int, summary: str, all_failures: list = None):
        ...
        _update_main_message(
            ...
            run_id=sub_run_id,
            platform_label=platform_key.upper(),
            summary=_format_summary_failures(summary, fs_lines),
        )
        with _runs_lock:
...
def _format_summary_failures(summary: str, fs_lines: list = None) -> str:
    lines = summary.split('\n')
    result = []
    in_failures = False
    ...
    for line in lines:
        if re.search(r'=+\s*(FAILURES|ERRORS)\s*=+', line):
            in_failures = True
            continue
        ...
        if in_short_summary:
            if re.match(r'(?:FAILED|ERROR)\s+', line):
                ...
                in_short_summary = False
                if fs_lines and not inserted:
                    clean = [re.sub(r'\s*\[.*?\]\s*$', '', l).strip() for l in fs_lines]
                    result.append('실패 내역 :')
                    result.extend(clean)
                    inserted = True
                result.append(line)
