...
class AOS_WEBTOON_GENRE:
    ...
    # 만화 장르홈과 동일한 범용 패턴(장르 무관 공통 UI) - 실기기 확인 결과 웹툰도 그대로 적용됨
    BIG_BANNER              = (AppiumBy.XPATH, '//android.view.ViewGroup[@clickable="true" and @content-desc!=""]')
    FOOTER                  = (AppiumBy.ANDROID_UIAUTOMATOR, 'new UiSelector().textContains("리디(주)")')
...
class IOS_WEBTOON_GENRE:
    ...
    SECTION_AI_TASTE        = (AppiumBy.IOS_CLASS_CHAIN, '**/XCUIElementTypeOther[`name CONTAINS "취향 저격"`]')
    # 로맨스 탭 전용 섹션(사용자가 알려준 명칭 기준) - 실기기 미확인, AOS와 동일 문구로 추정
    ...
    # BL 탭 전용 섹션(사용자가 알려준 명칭 기준) - 실기기 미확인, AOS와 동일 문구로 추정
    ...
    # 판타지/SF 탭 전용 섹션(사용자가 알려준 명칭 기준) - 실기기 미확인, AOS와 동일 문구로 추정
...
class AOS_WEBNOVEL_GENRE:
    ...
    # 기존에 다른 모듈이 쓰는 로케이터를 건드리지 않기 위해, 웹소설 전용 클래스 안에만 추가한다
    # (사용자 지시). 실기기 미확인 추정값이며, 검증 후 실제 문구가 다르면 여기 값만 고친다.
    #
    # 서브탭 - 웹툰(AOS_WEBTOON_GENRE)과 문구가 겹치는 것도 있지만 클래스가 달라 충돌 없음.
    SUBTAB_ROMANCE          = (AppiumBy.ANDROID_UIAUTOMATOR, 'new UiSelector().text("로맨스")')
    ...
    # 추천 탭 섹션 - 공백/조사 변화에 견디도록 textContains로 안전 매칭
    SECTION_SIMILAR_RECENT  = (AppiumBy.ANDROID_UIAUTOMATOR, 'new UiSelector().text("방금 본 작품과 비슷한")')
...
class IOS_WEBNOVEL_GENRE:
    ...
    # ---- 아래는 test_webnovel_genrehome.py용 신규 추가(2026-07-29) ----
    # 웹소설 장르홈은 iOS에서 화면 전체가 하나의 블롭으로 뭉쳐 노출되는 것이 이미 확인되어
    # (is_webnovel_genrehome_displayed 주석 참고, 2026-07-24), 개별 요소 매칭이 어렵다.
    # 그래서 만화/웹툰 iOS와 동일하게 XCUIElementTypeOther + CONTAINS 패턴으로 잡는다.
    # 실기기 미확인 추정값 - 검증 후 여기 값만 고친다.
    SUBTAB_ROMANCE          = (AppiumBy.ACCESSIBILITY_ID, '로맨스 로맨스')
    SUBTAB_ROMANCE_FANTASY  = (AppiumBy.ACCESSIBILITY_ID, '로판 로판')
    SUBTAB_BL               = (AppiumBy.ACCESSIBILITY_ID, 'BL BL')
    SUBTAB_FANTASY          = (AppiumBy.ACCESSIBILITY_ID, '판타지 판타지')
